...
void findNearestElements_and_NatrualCoordinates(
    double *pos, int &My_Chrono_Elem, ChVector<double> &Natural_coordinates,
...
void find_neighbor_nodes(std::vector<std::vector<int>> &my_neighbors,
                         ...
                         std::vector<ChVector<double>> nodal_corrd,
                         int total_num_nodes, double max_dist);

void writeMesh(std::shared_ptr<ChMesh> my_mesh, string SaveAs,
               std::vector<std::vector<int>> &NodeNeighborElement);
class cppChFlexPlate {
public:
  cppChFlexPlate(bool m_is3D, double m_timeStep, double *m_plate_center,
                 double *m_plate_dims, int *m_plate_num_div,
                 double *m_plate_prop, double *m_gravity, int *m_free_x)
      : is3D(m_is3D), time_step(m_timeStep), out_step(0.02), free_x(m_free_x) {

    const std::string rmCmd =
        (std::string("rm ") + data_folder + std::string("/*"));
    system(rmCmd.c_str());
    ...
    printf("plate_lenght_x=%f,plate_lenght_y=%f,plate_lenght_z=%f\n",
           plate_lenght_x, plate_lenght_y, plate_lenght_z);
    ...
    ChMatrixNM<double, 1, 3>
        Dims; // read element length, used in ChElementBrick
    Dims.setZero();
    ...
    for (int k = 0; k < numDiv_z; k++) {
      for (int j = 0; j < numDiv_y; j++) {
        for (int i = 0; i < numDiv_x; i++) {
          ...
          //                    printf("\nElement %d nodes are:
          //                    %d,%d,%d,%d,%d,%d,%d,%d,  ", num_elem, node0,
          //                    node1, node2, node3,
          //                           node4, node5, node6, node7);

          Element_nodes[num_elem].resize(8);
          Element_nodes[num_elem] = {node0, node1, node2, node3,
                                     node4, node5, node6, node7};

          ChVector<double> my_center =
              (1 / 8.0) * (nodal_corrd[Element_nodes[num_elem][0]] +
                           nodal_corrd[Element_nodes[num_elem][1]] +
                           nodal_corrd[Element_nodes[num_elem][2]] +
                           nodal_corrd[Element_nodes[num_elem][3]] +
                           nodal_corrd[Element_nodes[num_elem][4]] +
                           nodal_corrd[Element_nodes[num_elem][5]] +
                           nodal_corrd[Element_nodes[num_elem][6]] +
                           nodal_corrd[Element_nodes[num_elem][7]]);

          //                    printf("construct with center : %f,%f,%f\n",
          //                    my_center.x(), my_center.y(), my_center.z());

          element->SetNodes(
              std::dynamic_pointer_cast<ChNodeFEAxyz>(my_mesh->GetNode(node0)),
          ...
          element->SetGravityOn(
              false); // turn gravity on/off from within the element
          element->SetMooneyRivlin(
              false); // turn on/off Mooney Rivlin (Linear Isotropic by default)
          ChMatrixNM<double, 1, 9> stock_alpha_EAS; //
          stock_alpha_EAS.setZero();

          element->SetStockAlpha(stock_alpha_EAS(0, 0), stock_alpha_EAS(0, 1),
                                 stock_alpha_EAS(0, 2), stock_alpha_EAS(0, 3),
                                 stock_alpha_EAS(0, 4), stock_alpha_EAS(0, 5),
                                 stock_alpha_EAS(0, 6), stock_alpha_EAS(0, 7),
                                 stock_alpha_EAS(0, 8));
    ...
    find_neighbor_nodes(Neighbors, Element_nodes, nodal_corrd, TotalNumNodes,
                        Dims.maxCoeff());

    for (int i = 0; i < Neighbors.size(); i++) {
      if (Neighbors[i].size() < 6) {
        if (m_is3D ||
            (!m_is3D && std::abs(nodal_corrd[i].z()) < 1e-6
          ...
          printf("node %d, num neigh=%d, pos=%f,%f,%f\n", i,
                 Neighbors[i].size(), nodal_corrd[i].x(), nodal_corrd[i].y(),
                 nodal_corrd[i].z());
          num_surface_nodes++;
        }
      }
    ...
    nodal_corrd_vel_vec =
        (double *)malloc(num_total_nodes * 3 * sizeof(double *));
    ...
    auto msolver =
        std::static_pointer_cast<ChSolverMINRES>(my_system.GetSolver());
    ...
    //        auto mystepper =
    //        std::dynamic_pointer_cast<ChTimestepperHHT>(my_system.GetTimestepper());
  ...
  int *plate_num_div =
      new int[3]; //(Nx,Ny,Nz) number of division in each direction
  ...
  std::vector<std::vector<int>>
      Element_nodes; // Index of the nodes in an element
  std::vector<std::vector<int>>
      NodeNeighborElement; // Neighbor element of nodes in a vector
  std::vector<ChVector<double>>
      nodal_corrd_last; // Position of the nodes before the update
  ...
  double *step(double *forces, int num_nodes, double dt) {
    ...
    // // TODO: This averaged area is just a rough approximation for this
    // problem
    // // The pressure should be integrated based on some accurate average area
    // of each node. double dA = plate_dims[1]/plate_num_div[1] *
    // plate_dims[2]/plate_num_div[2];
    ...
    for (int i = 0; i < surface_nodes.size(); i++) {
      auto node = std::dynamic_pointer_cast<ChNodeFEAxyz>(
          my_mesh->GetNode(surface_nodes[i]));
      auto it =
          std::find(edge_nodes.begin(), edge_nodes.end(), surface_nodes[i]);
      ...
      ChVector<double> myForce = ChVector<double>(
  ...
  void writeThisFrame() {
    ...
    snprintf(SaveAsBuffer, sizeof(char) * 256,
             (data_folder + "flex_body.%d.vtk").c_str(), outframe);
    writeFrame(my_mesh, SaveAsBuffer, mesh_file, NodeNeighborElement);
    outframe++;
  }
  // This function must be called to update the std::vector representation of
  // the nodal coordinates This is important when a rank broadcast information
  // to others, and processor start calculating hx,hy,hz This must be called
  // before proceeding to hx,hy,hz
  ...
  void calcPos(ChMatrixDynamic<double> N, int Chrono_elem, ChVector<> &new_pos,
...

  {
    ...
    new_pos = N(0) * p0 + N(1) * p1 + N(2) * p2 + N(3) * p3 + N(4) * p4 +
              N(5) * p5 + N(6) * p6 + N(7) * p7;
  }
...

  {
    ...
    int node = thisElem[0];
    ChVector<> v0(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[1];
    ChVector<> v1(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[2];
    ChVector<> v2(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[3];
    ChVector<> v3(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[4];
    ChVector<> v4(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[5];
    ChVector<> v5(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[6];
    ChVector<> v6(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    node = thisElem[7];
    ChVector<> v7(nodal_corrd_vel_vec[3 * node + 0],
                  nodal_corrd_vel_vec[3 * node + 1],
                  nodal_corrd_vel_vec[3 * node + 2]);
    new_vel = N(0) * v0 + N(1) * v1 + N(2) * v2 + N(3) * v3 + N(4) * v4 +
              N(5) * v5 + N(6) * v6 + N(7) * v7;
  }
  void SaveNodalCor(std::vector<ChVector<double>> &nod_cor) {
    printf(" CHRONO: SaveNodalCor\n");
    for (int i = 0; i < my_mesh->GetNnodes(); i++) {
      nod_cor[i] = std::dynamic_pointer_cast<ChNodeFEAxyz>(my_mesh->GetNode(i))
                       ->GetPos();
    }
  }
  ...
  void calc_d_N_IBM(double *x, double *output) {
    ...
    for (int i = 0; i < surface_nodes.size(); i++) {
      ...
      if (L < d) {
        d = L;
        N = ChVector<>(Nodal_Normal[3 * i + 0], Nodal_Normal[3 * i + 1],
                       Nodal_Normal[3 * i + 2]);
  ...
  void calc_vel_IBM(double *x, double *output) {
    ...
    for (int i = 0; i < surface_nodes.size(); i++) {
      ...
      if (L < d && phi_s > 0) {
        d = L;
        closestnode = nodenum;
        ChVector<> vel = std::dynamic_pointer_cast<ChNodeFEAxyz>(
                             my_mesh->GetNode(closestnode))
                             ->GetPos_dt();
        output[0] = vel.x();
        output[1] = vel.y();
        output[2] = vel.z();
    ...
    //     findNearestElements_and_NatrualCoordinates(x, closestElem,
    //     Natural_coordinates, Element_nodes, nodal_corrd); printf("closest
    //     element to %f,%f,%f, is %d\n",x[0],x[1],x[2], closestElem); auto elem
    //     =
    //     std::dynamic_pointer_cast<ChElementBrick>(my_mesh->GetElement(closestElem));
    //     ChMatrixNM<double, 1, 8> N;
    //     elem->ShapeFunctions(N, Natural_coordinates.x(),
    //     Natural_coordinates.y(), Natural_coordinates.z());
    //     // printf("my elem=%d\t",closestElem);//
    //     Natural_coordinates=%f,%f,%f",closestElem,Natural_coordinates.x(),Natural_coordinates.y(),Natural_coordinates.z());
    //     ChVector<> out_vel(0);
...
cppChFlexPlate *newChFlexPlate(bool m_is3D, double m_timeStep,
                               double *m_plate_center, double *m_plate_dims,
                               int *m_plate_num_div, double *m_plate_prop,
                               double *m_gravity, int *m_free_x) {
  printf(" CHRONO: cppChFlexPlate *newChFlexPlate\n");
  return new cppChFlexPlate(m_is3D, m_timeStep, m_plate_center, m_plate_dims,
                            m_plate_num_div, m_plate_prop, m_gravity, m_free_x);
}

// This function gives a vector of points, searches in the chrono elements and
// return a vector of "the nearest element of each point"
void findNearestElements_and_NatrualCoordinates(
    double *pos, int &My_Chrono_Elem, ChVector<double> &Natural_coordinates,
    std::vector<std::vector<int>> elem_nodes,
    std::vector<ChVector<double>> nod_cor) {
  ...
  for (int i = 0; i < elem_nodes.size(); i++) {
    ChVector<double> my_center =
        (1 / 8.0) * (nod_cor[elem_nodes[i][0]] + nod_cor[elem_nodes[i][1]] +
                     nod_cor[elem_nodes[i][2]] + nod_cor[elem_nodes[i][3]] +
                     nod_cor[elem_nodes[i][4]] + nod_cor[elem_nodes[i][5]] +
  ...
  ChVector<double> x_dir = (+nod_cor[elem_nodes[nearest_elem][1]] +
                            nod_cor[elem_nodes[nearest_elem][2]] +
                            +nod_cor[elem_nodes[nearest_elem][5]] +
                            nod_cor[elem_nodes[nearest_elem][6]] +
                            -nod_cor[elem_nodes[nearest_elem][0]] -
                            nod_cor[elem_nodes[nearest_elem][3]] +
                            -nod_cor[elem_nodes[nearest_elem][4]] -
                            nod_cor[elem_nodes[nearest_elem][7]]) /
                           8.0;

  ChVector<double> y_dir = (+nod_cor[elem_nodes[nearest_elem][2]] +
                            nod_cor[elem_nodes[nearest_elem][3]] +
                            +nod_cor[elem_nodes[nearest_elem][6]] +
                            nod_cor[elem_nodes[nearest_elem][7]] +
                            -nod_cor[elem_nodes[nearest_elem][0]] -
                            nod_cor[elem_nodes[nearest_elem][1]] +
                            -nod_cor[elem_nodes[nearest_elem][4]] -
                            nod_cor[elem_nodes[nearest_elem][5]]) /
                           8.0;

  ChVector<double> z_dir = (+nod_cor[elem_nodes[nearest_elem][4]] +
                            nod_cor[elem_nodes[nearest_elem][5]] +
                            +nod_cor[elem_nodes[nearest_elem][6]] +
                            nod_cor[elem_nodes[nearest_elem][7]] +
                            -nod_cor[elem_nodes[nearest_elem][0]] -
  ...
  //    x_dir.y(), x_dir.z(), y_dir.x(), y_dir.y(),
  ...
  //    myNatural_coordinate.x(), myNatural_coordinate.y(),
...
void find_neighbor_nodes(std::vector<std::vector<int>> &my_neighbors,
  ...
  // Iterate through the elements nodes and figure out the neighbor nodes of
  // each node
  ...
  for (int i = 0; i < my_neighbors.size(); i++) {
    ...
    for (int j = 0; j < elem_nodes.size(); j++) {
      ...
      if (std::find(elem_nodes[j].begin(), elem_nodes[j].end(), i) !=
          elem_nodes[j].end()) {
        vector<int>::iterator it =
            find(elem_nodes[j].begin(), elem_nodes[j].end(), i);
        int pos = distance(elem_nodes[j].begin(), it);
        /////////////
        for (int k = 0; k < elem_nodes[j].size(); k++) {
          if (!(std::find(my_neighbors[i].begin(), my_neighbors[i].end(),
                          elem_nodes[j][k]) != my_neighbors[i].end())) {
            ///////////
